...
public class AppointmentRestController {
    ...
    public ResponseEntity<Map<String, Object>> update(@RequestHeader(value = "Authorization", required = false) String authorizationHeader,
                                             ...
                                             @RequestBody Map<String, Object> appointment) {
        log.debug("PUT /api/appointments/{} Authorization={} payload={}", id, authorizationHeader, appointment);
        if (authorizationHeader == null || !authorizationHeader.startsWith("Bearer ")) {
            log.warn("PUT /api/appointments/{} missing or invalid Authorization header", id);
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        String token = authorizationHeader.substring(7);
        Map<String, Object> updated = backendService.updateAppointment(token, id, appointment);
        log.debug("PUT /api/appointments/{} updated={}", id, updated);
    ...
    public ResponseEntity<Map<String, Object>> delete(@RequestHeader(value = "Authorization", required = false) String authorizationHeader,
                                               @PathVariable Integer id) {
        log.debug("DELETE /api/appointments/{} Authorization={}", id, authorizationHeader);
        if (authorizationHeader == null || !authorizationHeader.startsWith("Bearer ")) {
            log.warn("DELETE /api/appointments/{} missing or invalid Authorization header", id);
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        String token = authorizationHeader.substring(7);
        Map<String, Object> deleted = backendService.deleteAppointment(token, id);
        log.debug("DELETE /api/appointments/{} deleted={}", id, deleted);
